...
async def _init_pgvector(conn: asyncpg.Connection) -> None:
    # Register pgvector so we can bind Python lists/np arrays directly as vector
...
async def cosine_query(
    ...
    top_k: int,
    chunk_limit: int,
    # probes: int | None = None,  # enable if using ivfflat and want recall tuning
) -> List[int]:
    """
    Cosine distance (<=>) over chunk embeddings.
    Efficient version: fetch top chunk_limit chunks directly (index-friendly),
    then aggregate to doc-level using MIN(dist), then take top_k docs.
    """
    if query_vec is None:
        return []
    if hasattr(query_vec, "size") and query_vec.size == 0:  # numpy array
        return []
    ...
    WITH top_chunks AS (
        SELECT
            ac.doc_id,
            (ac.embedding_bge_m3 <=> $1::vector) AS dist
        FROM article_chunks ac
        WHERE ac.embedding_bge_m3 IS NOT NULL
          AND ac.doc_id IS NOT NULL
        ORDER BY ac.embedding_bge_m3 <=> $1::vector
        LIMIT $2
    ),
    doc_scores AS (
        SELECT doc_id, MIN(dist) AS best_dist
        FROM top_chunks
        GROUP BY doc_id
    )
    SELECT doc_id
    ...
    async with pool.acquire() as conn:
        # If you're using ivfflat, you can tune probes for better recall (slower).
        # This must be set per-session/connection.
...
async def evaluate(
    ...
    """
    Bounded evaluation: avoids creating 1 asyncio task per question.
    Uses N worker tasks that pull from a shared iterator.
    """
    ...
    print(
        f"Chunk semantic (cosine) @ {top_k}: macro F2={macro_f2:.4f} | "
...
async def main_async(
    ...
    print(
        f"Evaluating cosine similarity on {len(questions)} questions "
...
def main():
    parser = argparse.ArgumentParser(
        description="Chunk-level semantic retrieval (cosine distance) with async evaluation."
    )
