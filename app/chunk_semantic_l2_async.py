...
async def l2_query(
    ...
    """
    L2 distance (<->) over chunk embeddings.
    Fetch top chunk_limit chunks, aggregate min distance per doc, return top_k docs.
    """
...
async def evaluate(
    ...
    """
    Bounded evaluation: N worker tasks pull from a shared iterator.
    """
    ...
    print(
        f"Chunk semantic (L2) @ {top_k}: macro F2={macro_f2:.4f} | "
...
async def main_async(
    ...
    print(
        f"Evaluating L2 similarity on {len(questions)} questions "
...
def main():
    parser = argparse.ArgumentParser(
        description="Chunk-level semantic retrieval (L2 distance) with async evaluation."
    )
